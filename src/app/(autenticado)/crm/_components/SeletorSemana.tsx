...
function ehTerca(iso: string | null | undefined): boolean {
  if (!iso) return false
  const d = new Date(iso)
  return d.getDay() === 2
}
    ...
    >
      ...
      <div className="flex-1 text-center">
        <div className="flex items-center justify-center gap-3 flex-wrap">
          ...
          <p className="text-lg font-bold" style={{ color: '#FFFFFF' }}>SEMANA {semana}</p>
          {isAtual && (
            <span
      ...
      {!isAtual && (
        <button
          style={{ ...btnBase, color: '#C9A84C', borderColor: '#C9A84C44' }}
          disabled={carregando}
      ...
      {/* Navegação direita */}
      <button
        style={{ ...btnBase, opacity: semana >= semanaAtual ? 0.4 : 1, cursor: semana >= semanaAtual ? 'not-allowed' : (carregando ? 'not-allowed' : 'pointer') }}
