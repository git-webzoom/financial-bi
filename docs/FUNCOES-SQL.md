...
| `get_semana_webnario_ativa` | — | int | Semana de **webinário** ativa. **Calculada** (mesma fórmula de `listar_semanas_vendas`: `ultima_ocorrencia_brt(config 'webn')` ancorado em `webinario_semanas`, **−1** proposital). Lida **tanto pelo front (`/webnario`) quanto pelo webhook** → ambos viram juntos quando a `config 'webn'` cruza o horário (Ter **19:40** BRT desde 2026-06-10). **NÃO lê** a linha física de `webinario_semanas_presencas`. *(Antes lia a linha física e bugava — ver CHANGELOG 2026-06-02.)* |
