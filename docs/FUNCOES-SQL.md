...
| `get_lead_score_distribuicao_semana` | `p_numero int` | json | Distribuição de Lead Score por nota dos **captados de uma semana** (para o gráfico da aba Webinário do `/dashboard`). Cruza `webinario_inscritos(numero_semana=p_numero)` ⨝ `lead_score` por `contato_id` (faixa NULL = captado que não preencheu). Retorna `{total_captados, preencheram, sem_resposta, faixas:{A+,A,B,C,D}}`. `SECURITY DEFINER`. |

## Semanas (captação vs webinário — distintos!)
| Função | Args | Retorno | O que faz |
|--------|------|---------|-----------|
| `get_semana_atual` | — | int | Semana de **captação (CRM)** atual. |
| `get_semana_webnario_ativa` | — | int | Semana de **webinário** ativa. **Calculada** (mesma fórmula de `listar_semanas_vendas`: `ultima_ocorrencia_brt(config 'webn')` ancorado em `webinario_semanas`, **−1** proposital). Lida **tanto pelo front (`/webnario`) quanto pelo webhook** → ambos viram juntos quando a `config 'webn'` cruza o horário (Ter **19:00** BRT desde 2026-07-07; era 19:40 desde 2026-06-10). Antecipada porque acessos ao vivo que chegavam antes da virada caíam na semana anterior (bug de atribuição — CHANGELOG 2026-07-07). **NÃO lê** a linha física de `webinario_semanas_presencas`. *(Antes lia a linha física e bugava — ver CHANGELOG 2026-06-02.)* |
...
| `ensure_semana_existe` | `p_numero int` | void | Cria a semana na régua MASTER (`webinario_semanas`) se faltar — idempotente (`ON CONFLICT DO NOTHING`; só cria se a anterior `p_numero-1` existir). Chamada pelo cron `ensure-proxima-semana` **e** pela blindagem do `webhook-hotwebnar` (2026-06-10) para garantir a régua antes de gravar presença. |
| `ensure_semana_webnario_existe` | `p_numero int` | void | **No-op desde 2026-06-02** (mantida só porque o cron a chama). A ser removida na limpeza Fase 2 (ver `PENDENCIAS.md` 1b) — o FK das presenças já não depende mais de `webinario_semanas_presencas`. |
...
| `get_trafego_ads_aba` | `p_inicio date, p_fim date, p_regras jsonb='[]', p_logica text='and'` | TABLE | **Performance por anúncio** das abas `venda_direta` (TPW/Desafio/futuras). Agrega `trafego` por `ad_name` no período e calcula: `tipo` (video se `video_views_3s>0` ou `video_watches_75>0`, senão imagem — **prioriza o dado real sobre o nome do anúncio**), `gasto`, `impressoes`, `cpm`, `hook_rate` (=`video_views_3s/impressions`, NULL p/ imagem), `hold_rate` (=`video_watches_75/video_views_3s`, NULL p/ imagem), `click_rate` (vídeo: `link_clicks/video_watches_75`; imagem: CTR `link_clicks/impressions`), + cru `link_clicks/checkouts_initiated/video_views_3s/video_watches_75`. `p_regras` = mesmas regras de `filtros_personalizados_regras` do filtro de **tráfego** da aba (JSONB `[{campo,operador,valor,ordem}]`), aplicadas genericamente espelhando `aplicarRegras`. **`p_logica`** (`'and'`/`'or'`, default `'and'`) = a coluna `filtros_personalizados.logica` do filtro: combina as regras entre si com AND ou OR; o bloco de regras entra **sempre em AND** com o WHERE de data. **Segurança:** campo validado contra whitelist (`campaign_name/adset_name/ad_name/ad_account_id`); valores via `format %L` (sem injection); `p_logica` mapeado por CASE→literal `' AND '`/`' OR '` (sem injection). `HAVING` descarta anúncios sem gasto e sem impressão. `SECURITY DEFINER`, `EXECUTE` p/ `authenticated`. **Lado VENDA (R$/nº/CPA/ROAS) NÃO entra aqui** — fica agregado na aba (vendas não carregam o anúncio; `utm_content` vazio em ~99%). |
