-- No ON CONFLICT (mesma transação, status diferente), atualizar apenas:
-- status, motivo_reembolso, data_cancelamento, data_garantia, raw_id, updated_at.
-- Todos os demais campos (valores, contato, produto, oferta, UTMs, pagamento)
-- ficam como vieram no primeiro evento de criação.
...
BEGIN
  ...
  ON CONFLICT (id) DO UPDATE SET
    -- Apenas campos de estado da transação são atualizados
    ...
    updated_at        = now();
  -- Valores, contato, produto, oferta, UTMs, pagamento, datas de pedido/aprovação
  -- ficam intactos do primeiro evento.
