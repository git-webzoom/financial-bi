-- Corrige process_venda: lê invoice.cycle do payload para assinatura_ciclo
-- Antes estava sempre inserindo NULL nesse campo
...
BEGIN
  ...
  -- Lê o ciclo da assinatura: invoice.cycle tem prioridade, depois subscription.charged_times
...
-- Reprocessa todas as vendas existentes para popular assinatura_ciclo a partir do raw
UPDATE vendas v
SET assinatura_ciclo = COALESCE(
  NULLIF(rv.payload->'invoice'->>'cycle', '')::int,
  NULLIF(rv.payload->'subscription'->>'charged_times', '')::int
)
FROM raw_vendas rv
WHERE rv.id = v.raw_id
  AND v.assinatura_ciclo IS NULL;
