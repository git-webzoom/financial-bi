-- ============================================================
-- Corrige process_venda:
-- 1. pptc: trata caso em que source.pptc não é array
-- 2. oferta_id: ignora FK se oferta não existe no banco
-- ============================================================
...
BEGIN
  ...
  -- oferta_id: só usa se existir na tabela ofertas (evita FK violation)
  v_oferta_id := (v_payload->'product'->'offer'->>'id')::uuid;
  IF NOT EXISTS (SELECT 1 FROM ofertas WHERE id = v_oferta_id) THEN
    v_oferta_id := NULL;
  END IF;

  -- rppc: trata pptc que pode ser null, objeto ou array
  v_pptc := v_payload->'source'->'pptc';
  v_rppc := CASE
    WHEN v_pptc IS NULL THEN NULL
    WHEN jsonb_typeof(v_pptc) = 'array' AND jsonb_array_length(v_pptc) > 0
      THEN CASE
